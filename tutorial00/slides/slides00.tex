...
\end{array}$};
\node at (10,0.5) {$\mathcal{I}_h^{int}=\{1\}, \mathcal{I}_h^{\partial\Omega}=\{2,3,4,5\}$};
\end{tikzpicture}
\end{center}
\begin{itemize}
\item A mesh consists of \textit{ordered} sets of vertices and elements:
\begin{equation*}
\mathcal{X}_h = \{x_1,\ldots,x_N\} \subset \mathbb{R}^d, \quad
\mathcal{T}_h = \{T_1, \ldots, T_M\}
\end{equation*}
\item {\em Simplicial element}: $T=\text{interior}(\text{convex\_hull}(x_{T,0},\ldots,x_{T,d}))$
\item {\em Conforming}: Intersection of boundary is subentity
...
\item This may become inefficient for {\em sparse} linear systems
...
except (3) which needs to be supplied by the implementer of a FEM
