...
same amount of time (few multi-core CPUs)
...
to other processors or other parts of the same processor (most multi-core CPUs, multi-processor servers)
\end{itemize}
...
\begin{itemize}
\item Users view: Copy (contiguous) memory block from one address space to 
  another.
...
\begin{center}
\includegraphics[width=0.7\textwidth]{dnapl-3d-het-iso}\\
...
\begin{frame}
  ...
  \item<1-> \lstinline!YaspGrid!
      ...
      \end{itemize}
  \item<2-> Unstructured 2D/3D grids with full load-balancing:
  \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \item UGGrid and 3D ALUGrid provide methods to re-balance the work load after grid adaptation
...
where
    ...
    \item \lstinline!CommDataHandleIF!\\
      ...
      source process and distribute the data on the target process (see exercises).
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item parallel computations require global communication (e.g. sum(defect) or $\min(\Delta t)$
    and synchronization, e.g. a barrier needed for timing)
  \item You can get a collective communication object by the following method of a \lstinline!GridView!:
\begin{lstlisting}
...
\only<presentation>{\lstset{basicstyle=\scriptsize\ttfamily}}    \begin{tabular}{l|p{6cm}}
      ...
      \lstinline!getCommunication! & get collective communication object for \lstinline!MPI_COMM_WORLD!\\
